...
function timeSpanToString(startDate, endDate) {
  ...
  const hours = Math.floor(timeSpanInMs / 3600000);
  const minutes = Math.floor(timeSpanInMs / 60000) % 60;
  const seconds = Math.floor(timeSpanInMs / 1000) % 60;
  let miliseconds = timeSpanInMs % 1000;

  const elements = [hours, minutes, seconds];
  const output = elements.map((el) => (el < 10 ? `0${el}` : `${el}`));
...
function angleBetweenClockHands(date) {
  ...
  let angleBetweenHands = Math.abs((60 * hours - 11 * minutes) / 2);
