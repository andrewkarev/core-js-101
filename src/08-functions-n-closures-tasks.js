...
function logger(func, logFunc) {
  return (...args) => {
    ...
    const argsList = JSON.stringify(args).slice(1, -1);
    logFunc(`${funcName}(${argsList}) starts`);
    const result = func(...args);
    logFunc(`${funcName}(${argsList}) ends`);
    return result;
  };
}
...
function partialUsingArguments(fn, ...args1) {
  const firstPart = args1;
  return (...args2) => {
    const argsTotal = firstPart.concat(args2);
    return fn(...argsTotal);
  };
}
